...
import { StaffOrganisation, UpdateStaffOrganisationRequest, ProductModule } from '@/types';
...
export default function OrganisationDetailPage() {
  ...
  const fetchOrganisation = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        allowed_auth_methods: data.allowed_auth_methods || ['local'],
      });
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      // Include owner if changed
      const updateData: UpdateStaffOrganisationRequest = {
        ...formData,
      };
      
      // Only include owner if it's different from current owner
      const newOwnerIdNum = newOwnerId ? parseInt(newOwnerId) : null;
      if (newOwnerIdNum !== organisation?.owner) {
        if (newOwnerIdNum) {
          updateData.owner = newOwnerIdNum;
        }
        // If newOwnerId is empty and there was an owner, we can't remove it via API
        // The API documentation doesn't mention removing owner, so we'll only update if a new ID is provided
      }
            ...
            {!isEditing ? (
              <>
                ...
                <dl className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  ...
                  <div>
                    <dt className="text-sm font-medium text-gray-500">Allowed Auth Methods</dt>
                    <dd className="mt-1">
                      <div className="flex flex-wrap gap-2">
                        {organisation.allowed_auth_methods?.map((method) => (
                          <span key={method} className="px-2 py-1 text-xs bg-gray-100 text-gray-800 rounded">
                            {method}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <dd className="mt-1 text-sm text-gray-900">
                      ...
                      <p className="mt-1 text-xs text-gray-500">
                        The core <span className="font-mono">{ORGANISATION_CORE_MODULE_KEY}</span> module is always on. Staff-only; tenants cannot change this via the client API.
                      </p>
                    </dd>
                  </div>
                  <div>
              ...
              <>
                    ...
                    <button
                      onClick={() => {
                        ...
                        setFormData({
                          ...
                          allowed_auth_methods: organisation.allowed_auth_methods || ['local'],
                        });
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    />
                    <p className="mt-1 text-xs text-gray-500">
                      Enter the ID of an existing account with account_type="business_owner" to change the owner. Leave empty to keep current owner.
                    </p>
                  </div>
